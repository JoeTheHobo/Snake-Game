...
function movePlayers() {
    for (let i = 0; i < playersInServer.length; i++) {
        ...
        console.log(player.moveTik, deltaTime, player.moveSpeed,currentBoard.map,player.pos.x,player.pos.y);
...
function endScreen() {
    ...
    let longestTail = playersInServer[0].longestTail;
    let timeSurvived = playersInServer[0].timeSurvived;
    let mostKills = playersInServer[0].playerKills;

    let longestTailPlayer = playersInServer[0];
    let timeSurvivedPlayer = playersInServer[0];
    let mostKillsPlayer = playersInServer[0];
    for (let i = 1; i < playersInServer.length; i++) {
        if (playersInServer[i].longestTail > longestTail) {
            longestTail = playersInServer[i].longestTail;
            longestTailPlayer = playersInServer[i];
        }
        if (playersInServer[i].timeSurvived > timeSurvived) {
            timeSurvived = playersInServer[i].timeSurvived;
            timeSurvivedPlayer = playersInServer[i];
        }
        if (playersInServer[i].playerKills > mostKills) {
            mostKills = playersInServer[i].mostKills;
            mostKillsPlayer = playersInServer[i];
        }
    }
    ...
    if (playersInServer.length > 1 && mostKills > 0) {
...
function deletePlayer(player,playerWhoKilled,item){
    ...
    if (player.shield < 0){
        ...
        let playersDead = 0;
        for (let i = 0; i < playersInServer.length; i++) {
            if (playersInServer[i].isDead) playersDead++;
        }
        if (playersDead == playersInServer.length) {
            endScreen();
        }
        return;
    }
...
function startGame() {
    ...
    for (let i = 0; i < playersInServer.length; i++) {
        ...
        if (player.pos.x == false)        
            spawn(player);
    }

    setUpPlayerCanvas();
    setUpItemCanvas(); //add canvas
    adjustCanvasSize();
    renderGame();
